module GrowthCurveSampleHelper
  ...
  def age_slide_link(direction:)
    is_over_prev_edge = (0..AGE_SLIDE).cover?(@current_max_age)
    is_over_next_edge = params[:current_set_age].to_i > @max_age

    a_link_or_plain_text =
    ...
    case direction
    when :prev
      a_link_or_plain_text.call('<', is_over_prev_edge, :-)
    when :next
      a_link_or_plain_text.call('>', is_over_next_edge, :+)
    end
  end
